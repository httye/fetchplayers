# UserInfoAPI - Minecraft Paper 插件 v2.0

UserInfoAPI 是一个为 Minecraft Paper 服务器开发的插件，提供 REST API 接口来获取玩家信息。**安装即用，零配置启动！**

**作者**: httye
**项目**: UserInfoAPI

## 🚀 快速开始

### 1. 安装插件
1. 下载 `UserInfoAPI-2.0.jar` 文件
2. 将 JAR 文件放入服务器的 `plugins` 文件夹
3. 启动服务器，插件会自动生成配置文件
4. **完成！** API服务器会自动启动在 `http://localhost:8080`

### 2. 测试API
启动服务器后，立即测试API是否正常工作：
```bash
curl "http://localhost:8080/api/status"
```

如果看到 `{"status":"online","plugin":"UserInfoAPI","version":"2.0"}`，说明安装成功！

### 3. PHP调用示例
```php
// 包含简易客户端
include 'minecraft_api.php';

// 获取玩家信息
$player = getPlayerInfo('Steve');
echo "玩家等级: " . $player['level'];

// 获取在线玩家
$online = getOnlinePlayers();
echo "在线人数: " . $online['count'];

// 检查玩家是否在线
if (isPlayerOnline('Steve')) {
    echo "Steve 在线！";
}
```

## 📋 功能特性

- ✅ **零配置启动** - 插件安装后立即可用
- ✅ **批量查询** - 一次性查询多个玩家信息
- ✅ **离线玩家支持** - 即使玩家离线也能获取基本信息
- ✅ **数据导出** - 支持JSON/CSV格式导出
- ✅ **API限流** - 防止API被滥用
- ✅ **智能缓存** - 提高响应速度
- ✅ **详细日志** - 便于调试和监控
- ✅ **PHP友好** - 优化的JSON格式和CORS支持
- ✅ **错误恢复** - 自动修复配置问题

## 🔧 API 端点

### 基础端点
| 端点 | 方法 | 描述 |
|------|------|------|
| `/api/status` | GET | 获取服务器状态 |
| `/api/user/info?username=Steve` | GET | 获取玩家完整信息 |
| `/api/user/level?username=Steve` | GET | 获取玩家等级信息 |
| `/api/user/location?username=Steve` | GET | 获取玩家位置信息 |
| `/api/user/inventory?username=Steve` | GET | 获取玩家背包信息 |
| `/api/online-players` | GET | 获取在线玩家列表 |

### 高级端点
| 端点 | 方法 | 描述 |
|------|------|------|
| `/api/user/batch` | POST | 批量查询玩家信息 |
| `/api/export?type=players&format=json` | GET | 导出玩家数据 |
| `/api/user/login-records?username=Steve` | GET | 获取玩家登录记录 |
| `/api/security/info` | GET | 获取安全系统信息 |
### 批量查询示例
```bash
curl -X POST "http://localhost:8080/api/user/batch?api_key=你的密钥" \
  -H "Content-Type: application/json" \
  -d '{"usernames":["Steve","Alex"],"queryType":"info"}'
```

### 离线玩家查询
现在支持查询离线玩家的基本信息（需要玩家曾经加入过服务器）：
- **在线玩家**: 返回实时数据，`online` 字段为 `true`
- **离线玩家**: 返回历史数据，`online` 字段为 `false`
- **从未加入的玩家**: 返回错误信息

示例响应：
```json
{
  "username": "Steve",
  "uuid": "8667ba71b85a407a960c8e4e30a6b02d",
  "level": 0,
  "exp": 0.0,
  "health": 20.0,
  "foodLevel": 20,
  "gameMode": "SURVIVAL",
  "online": false,
  "firstPlayed": 1640995200000,
  "lastPlayed": 1640995200000,
  "isOnline": false,
  "whitelisted": false,
  "banned": false,
  "op": false,
  "currentSessionOnlineTime": 0,
  "totalOnlineTime": 3600
}
```

### 在线时长功能
新增当次在线时长和总在线时长统计：
- **currentSessionOnlineTime**: 当前会话在线时长（秒）
- **totalOnlineTime**: 玩家总在线时长（秒）

## 🔑 安全认证

插件默认关闭安全功能，方便快速开始使用。生产环境建议启用：

```yaml
security:
  enabled: true
  allowed-ips:
    - "127.0.0.1"
    - "192.168.1.0/24"
  api-keys:
    - key: "你的安全密钥"
      name: "生产环境密钥"
      active: true
```

## ⚙️ 配置文件

插件首次启动时会自动生成配置文件 `plugins/UserInfoAPI/config.yml`：

```yaml
# API服务器配置
api:
  host: "0.0.0.0"
  port: 8080

# 安全设置（默认关闭）
security:
  enabled: false

# 限流设置（默认启用）
rate-limit:
  enabled: true
  requests-per-minute: 60
```

## 🎮 游戏内命令

- `/userinfo status` - 查看插件状态和API信息
- `/userinfo reload` - 重载配置文件
- `/userinfo key generate [名称]` - 生成新的API密钥
- `/userinfo help` - 显示帮助信息

**权限要求**: `userinfoapi.admin`

## 📊 返回数据格式

### 玩家信息示例
```json
{
  "username": "Steve",
  "level": 10,
  "exp": 0.5,
  "location": {
    "x": 100.5,
    "y": 64.0,
    "z": -200.3,
    "world": "world"
  },
  "health": 20.0,
  "gameMode": "SURVIVAL"
}
```

### 在线玩家示例
```json
{
  "count": 3,
  "players": [
    {
      "username": "Steve",
      "displayName": "Steve",
      "onlineTime": 3600
    }
  ]
}
```

## 🛠️ 编译方法

如果需要自行编译：
```bash
# 确保安装了 Java 17+ 和 Maven
mvn clean package
# 生成的JAR文件在 target/ 目录
```

## 📁 文件说明

```
UserInfoAPI/
├── UserInfoAPI-2.0.jar          # 插件主文件
├── simple_php_client.php        # 简易PHP客户端
├── php_client_library.php       # 完整PHP客户端库
├── php_example.php             # PHP使用示例
├── README.md                   # 本文档
└── config.yml                  # 配置文件模板
```

## 🔍 故障排除

### API无法访问
1. 检查端口是否被占用：`netstat -an | findstr 8080`
2. 检查防火墙设置
3. 查看服务器控制台是否有错误信息

### 获取不到玩家信息
1. 确保玩家在线
2. 检查玩家名拼写
3. 查看服务器控制台日志

### PHP调用失败
1. 检查API密钥是否正确
2. 确保服务器地址正确
3. 检查PHP的cURL扩展是否启用

## 🔧 高级配置

### 限流设置
```yaml
rate-limit:
  enabled: true
  requests-per-minute: 60    # 每分钟最多60次请求
  requests-per-hour: 1000    # 每小时最多1000次请求
```

### 批量查询设置
```yaml
batch-query:
  enabled: true
  max-players: 50           # 单次最多查询50个玩家
```

### 数据导出设置
```yaml
data-export:
  enabled: true
  max-records: 1000         # 最多导出1000条记录
```

## 📈 性能优化

- **启用缓存**: 减少重复查询
- **合理限流**: 防止API过载
- **批量查询**: 减少请求次数
- **选择必要字段**: 避免获取不需要的数据

## 🤝 支持与反馈

如有问题或建议：
- 提交 Issue
- 查看服务器控制台日志
- 检查配置文件语法

## 📄 许可证

此插件遵循 MIT 许可证。详见 LICENSE 文件。

---

**作者**: httye
**项目**: UserInfoAPI
**版本**: 2.0

**享受使用 UserInfoAPI！** 🎉